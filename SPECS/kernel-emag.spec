...
%define all_arch_configs %{src_pkg_name}-%{version}-emag.config
...
BuildKernel() {
    ...
    Config=%{src_pkg_name}-%{version}-emag.config
...
%define __modsign_install_post \
  if [ "%{with_debug}" -ne "0" ]; then \
    ...
    Arch=`head -1 configs/%{src_pkg_name}-%{version}-emag.config | cut -b 3-` \
    rm -rf .tmp_versions \
